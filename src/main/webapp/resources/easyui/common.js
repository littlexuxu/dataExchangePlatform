...
function form2Json(formid) {
    ...
    $("#"+formid).find('input[type="checkbox"]').each(function () {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            {
                arr[i].value = document.getElementById(this.id).checked?"1":"0";
                has = true;
            }
        }
        if (!has)
        {
            arr.push({ name: this.id, value: document.getElementById(this.id).checked ? "1" : "0" });
        }
    })
    jsonStr += '{';
    for (var i = 0; i < arr.length; i++) {
...
function CenterWin(linkStyle, isDialog) {
    if (!linkStyle) return "";
    if (isDialog)
        var sp = new StringParam(linkStyle.toString().toLowerCase());
    else
        var sp = new StringParam(linkStyle.toString().toLowerCase(), ",", "=");
...
var MaskUtil = (function () {
    ...
    function init() {
        if (!$mask) {
            $mask = $("<div class=\"datagrid-mask mymask\" style='z-index:999999998'></div>").appendTo("body");
        }
        if (!$maskMsg) {
            $maskMsg = $("<div class=\"datagrid-mask-msg mymask\" style='z-index:999999999;height:40px;'>" + defMsg + "</div>")
...
function disableForm(formId, isDisabled) {
    ...
    $("#" + formId).find('.combo-f').each(function () {
        if (this.id) {
    ...
    $("#" + formId).find('.datebox-f').each(function () {
        if (this.id) {
...
function readonlyForm(formId, isDisabled) {
    ...
    $("#" + formId).find('datebox-f').each(function () {
        if (this.id) {
            $("#" + this.id).datebox("readonly", isDisabled);
        }
    });
}